...
public class WalletImpl implements IImportExportWallet<WalletDto> {
    ...
    public Try<WalletDto> extractWallet(String walletName, String passphrase) {
        return walletStorageService.extractWallet(walletName, passphrase).map(wallet -> new WalletDto(wallet).setNameId(walletName).setPassphrase(passphrase));
    }

    public Try<WalletDto> importWallet(String walletName, String mnemonicString, String passphrase) {
        return walletStorageService.importWallet(walletName, mnemonicString, passphrase).map(wallet -> new WalletDto(wallet).setNameId(walletName).setPassphrase(passphrase));
    }
